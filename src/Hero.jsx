...
const Hero = () => {
  return (
    ...
    >
      ...
      {/* Kept your 50% white overlay */}
      <div className="absolute inset-0 bg-white/50 z-0"></div>
      
      {/* Left Text Content */}
      <div className="max-w-2xl relative z-10 md:w-1/2">
        {/* Slightly tightened the text sizes (text-7xl to 6xl) so it breathes better in the new height */}
        <h1 className="text-5xl md:text-6xl font-anton text-krossover-blue mb-4 leading-tight uppercase drop-shadow-sm">
          Your Safety and Comfort, <br />
          <span className="text-gray-800">Our Style.</span>
        </h1>
        
        <p className="text-base md:text-lg font-poppins text-gray-700 mb-8 leading-relaxed font-medium">
        ...
        <div className="flex flex-col sm:flex-row gap-4 font-poppins">
          ...
          <a href="/services" className="bg-white hover:bg-gray-100 border-2 border-krossover-blue text-krossover-blue text-lg px-8 py-3 rounded-md font-semibold shadow-sm transition text-center flex items-center justify-center">
            Explore Fleet
          </a>
        </div>
      </div>
      ...
      {/* Reduced mt-12 to mt-8, and md:h-[500px] to md:h-[450px] for better viewport fit */}
      <div className="md:w-1/2 mt-8 md:mt-0 relative w-full h-[300px] md:h-[450px] z-10">
        <div className="absolute inset-0 animate-drive-in flex items-center justify-center">
          {/* Kept your specific Sleek Transit Bus image */}
          <img 
